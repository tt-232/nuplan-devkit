...
class Image(Base):
    ...
    next_token = Column(
        sql_types.HexLen8, ForeignKey("image.token"), nullable=True
    )  # type: str
    prev_token = Column(
        sql_types.HexLen8, ForeignKey("image.token"), nullable=True
    )  # type: str
    ...
    camera_token = Column(
        sql_types.HexLen8, ForeignKey("camera.token"), nullable=False
    )  # type: str
    filename_jpg = Column(String(128))  # type: str
    timestamp = Column(Integer)  # type: int

    next = relationship(
        "Image", foreign_keys=[next_token], remote_side=[token]
    )  # type: Image
    prev = relationship(
        "Image", foreign_keys=[prev_token], remote_side=[token]
    )  # type: Image
    camera = relationship(
        "Camera", foreign_keys=[camera_token], back_populates="images"
    )  # type: Camera
    ego_pose = relationship(
        "EgoPose", foreign_keys=[ego_pose_token], back_populates="image"
    )  # type: EgoPose
    ...
    def lidar_pc(self) -> LidarPc:
        ...
        lidar_pc = (
            ...
            .order_by(func.abs(LidarPc.timestamp - self.timestamp))
            .first()
        )
        return lidar_pc
    ...
    def load_as(self, db: NuPlanDB, img_type: str) -> Any:
        ...
        assert img_type in [
            ...
            "cv2",
            "np",
        ], f"Expected img_type to be pil, cv2 or np. Received {img_type}"
    ...
    def load_bytes_jpg(self, db: NuPlanDB) -> BinaryIO:
        ...
        blob: BinaryIO = db.load_blob(
            osp.join("nuplan-v1.1/sensor_blobs", self.filename)
        )
        return blob
    ...
    def boxes(self, frame: Frame = Frame.GLOBAL) -> List[Box3D]:
        ...
        boxes: List[Box3D] = get_boxes(
            self, frame, self.ego_pose.trans_matrix_inv, self.camera.trans_matrix_inv
        )
        ...
        if direction == "prev":
            if mode == "n_poses":
                ...
                raise NotImplementedError(
                    "Only n_poses and n_seconds two modes are supported for now!"
                )
        elif direction == "next":
            if mode == "n_poses":
                ego_poses = (
                ...
                raise NotImplementedError(
                    "Only n_poses and n_seconds two modes are supported!"
                )
        else:
    ...
    def render(
        ...
        ax.imshow(self.load_as(db, img_type="pil"), alpha=0.5)
        ...
        if with_3d_anns:
            for box in self.boxes(Frame.SENSOR):
                ...
                color = (
                    ...
                    c,
                    np.array([0, 0, 0]),
                )  # type: Tuple[Any, Any, np.typing.NDArray[np.int64]]

                if box_in_image(
                    box,
                    self.camera.intrinsic_np,
                    (self.camera.width, self.camera.height),
                    vis_level=box_vis_level,
                ):
                    box.render(
                        ax, view=self.camera.intrinsic_np, normalize=True, colors=color
                    )
...
Camera.images = relationship(
    "Image", foreign_keys="Image.camera_token", back_populates="camera"
)
EgoPose.image = relationship(
    "Image",
    foreign_keys="Image.ego_pose_token",
